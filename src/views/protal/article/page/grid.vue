...
<script>
	...
	import expandAttach from './expand/attach'
	...
	export default {
				...
				datagrid: {
					table: {
						columns: [{
							...
							{
								title:that.$t('article_attach'),
								width: 90,
								align: 'center',
								fixed: 'right',
								render: (h, params) => {
									return h(expandAttach, {
										props: {
											data: params.row.attach,
										}
									})
								}
							},
							{
								...
								fixed: 'right',
								render: (h, params) => {
									return h(expandImages, {
										props: {
											data: params.row.images,
										}
									})
								}
							},
		...
		methods: {
			...
			renderArticle(where, callback) {
				...
				this.$store.dispatch('readArticlePage', where).then((result) => {
			...
			deleteArticle(row, index) {
				...
				that.$Modal.confirm({
					...
					onOk: function() {
						that.$store.dispatch('deleteArticlePage', {
							uniqid: row.uniqid,
						}).then((result) => {
			...
			handleField(uniqid, field, value, callback) {
				this.$store.dispatch('updateArticlePage', {
					uniqid: uniqid,
					field: field,
					value: value
				}).then((result) => {
